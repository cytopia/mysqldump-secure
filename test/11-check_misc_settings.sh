...
sed_change_config_file "^TMP_DIR="  "#TMP_DIR="
	...
	echo "---------- CRON MODE ----------"
	CMD="${CMD_CRON}"
	if ! check "1" "1" "WARN" "1" "\$TMP_DIR" "1" "1" "0" "${CMD}"; then ERROR=$((ERROR+1)); fi

	echo "---------- NORMAL MODE ----------"
	CMD="${CMD_NORM}"
	if ! check "1" "1" "WARN" "1" "\$TMP_DIR" "1" "1" "1" "${CMD}"; then ERROR=$((ERROR+1)); fi
...
sed_change_config_file "^TMP_DIR=\"/tmp\""  "TMP_DIR=\"/foo\""
	...
	echo "---------- CRON MODE ----------"
	CMD="${CMD_CRON}"
	if ! check "1" "1" "ABORT" "1" "\$TMP_DIR" "1" "1" "0" "${CMD}"; then ERROR=$((ERROR+1)); fi

	echo "---------- NORMAL MODE ----------"
	CMD="${CMD_NORM}"
	if ! check "1" "1" "ABORT" "1" "\$TMP_DIR" "1" "1" "1" "${CMD}"; then ERROR=$((ERROR+1)); fi
