...
sed_change_config_file "^DELETE=1"  "#DELETE=1"
	...
	echo "---------- CRON MODE ----------"
	CMD="${CMD_CRON}"
	if ! check "1" "1" "WARN" "1" "\$DELETE" "1" "1" "0" "4" "${CMD}"; then ERROR=$((ERROR+1)); fi

	echo "---------- NORMAL MODE ----------"
	CMD="${CMD_NORM}"
	if ! check "1" "1" "WARN" "1" "\$DELETE" "1" "1" "1" "4" "${CMD}"; then ERROR=$((ERROR+1)); fi
...
sed_change_config_file "^DELETE=1"  "DELETE=2"
	...
	echo "---------- CRON MODE ----------"
	CMD="${CMD_CRON}"
	if ! check "1" "1" "ERR" "1" "\$DELETE" "1" "1" "0" "4" "${CMD}"; then ERROR=$((ERROR+1)); fi

	echo "---------- NORMAL MODE ----------"
	CMD="${CMD_NORM}"
	if ! check "1" "1" "ERR" "1" "\$DELETE" "1" "1" "1" "4" "${CMD}"; then ERROR=$((ERROR+1)); fi
...
sed_change_config_file "^DELETE_METHOD=\"${TMPWATCH}\""  "#DELETE_METHOD=\"${TMPWATCH}\""
	...
	echo "---------- CRON MODE ----------"
	CMD="${CMD_CRON}"
	if ! check "1" "1" "WARN" "1" "\$DELETE_METHOD" "1" "1" "0" "4" "${CMD}"; then ERROR=$((ERROR+1)); fi

	echo "---------- NORMAL MODE ----------"
	CMD="${CMD_NORM}"
	if ! check "1" "1" "WARN" "1" "\$DELETE_METHOD" "1" "1" "1" "4" "${CMD}"; then ERROR=$((ERROR+1)); fi
...
sed_change_config_file "^DELETE_METHOD=\"${TMPWATCH}\""  "DELETE_METHOD=\"wrong\""
	...
	echo "---------- CRON MODE ----------"
	CMD="${CMD_CRON}"
	if ! check "1" "1" "ERR" "1" "\$DELETE_METHOD" "1" "1" "0" "4" "${CMD}"; then ERROR=$((ERROR+1)); fi

	echo "---------- NORMAL MODE ----------"
	CMD="${CMD_NORM}"
	if ! check "1" "1" "ERR" "1" "\$DELETE_METHOD" "1" "1" "1" "4" "${CMD}"; then ERROR=$((ERROR+1)); fi
...
sed_change_config_file "^DELETE_FORCE=1"  "#DELETE_FORCE=1"
	...
	echo "---------- CRON MODE ----------"
	CMD="${CMD_CRON}"
	if ! check "1" "1" "WARN" "1" "\$DELETE_FORCE" "1" "1" "0" "4" "${CMD}"; then ERROR=$((ERROR+1)); fi

	echo "---------- NORMAL MODE ----------"
	CMD="${CMD_NORM}"
	if ! check "1" "1" "WARN" "1" "\$DELETE_FORCE" "1" "1" "1" "4" "${CMD}"; then ERROR=$((ERROR+1)); fi
...
sed_change_config_file "^DELETE_FORCE=1"  "DELETE_FORCE=2"
	...
	echo "---------- CRON MODE ----------"
	CMD="${CMD_CRON}"
	if ! check "1" "1" "ERR" "1" "\$DELETE_FORCE" "1" "1" "0" "4" "${CMD}"; then ERROR=$((ERROR+1)); fi

	echo "---------- NORMAL MODE ----------"
	CMD="${CMD_NORM}"
	if ! check "1" "1" "ERR" "1" "\$DELETE_FORCE" "1" "1" "1" "4" "${CMD}"; then ERROR=$((ERROR+1)); fi
...
sed_change_config_file "^DELETE_FORCE=1"  "DELETE_FORCE=0"
	...
	echo "---------- CRON MODE ----------"
	CMD="${CMD_CRON}"
	if ! check "1" "1" "WARN" "1" " skipped: " "1" "1" "0" "4" "${CMD}"; then ERROR=$((ERROR+1)); fi

	echo "---------- NORMAL MODE ----------"
	CMD="${CMD_NORM}"
	if ! check "1" "1" "WARN" "1" " skipped: " "1" "1" "1" "4" "${CMD}"; then ERROR=$((ERROR+1)); fi
...
sed_change_config_file "^DELETE_IF_OLDER=1m"  "#DELETE_IF_OLDER=1m"
	...
	echo "---------- CRON MODE ----------"
	CMD="${CMD_CRON}"
	if ! check "1" "1" "WARN" "1" "\$DELETE_IF_OLDER" "1" "1" "0" "4" "${CMD}"; then ERROR=$((ERROR+1)); fi

	echo "---------- NORMAL MODE ----------"
	CMD="${CMD_NORM}"
	if ! check "1" "1" "WARN" "1" "\$DELETE_IF_OLDER" "1" "1" "1" "4" "${CMD}"; then ERROR=$((ERROR+1)); fi
...
sed_change_config_file "^DELETE_IF_OLDER=1m"  "DELETE_IF_OLDER=0"
	...
	echo "---------- CRON MODE ----------"
	CMD="${CMD_CRON}"
	if ! check "1" "1" "ERR" "1" "\$DELETE_IF_OLDER" "1" "1" "0" "4" "${CMD}"; then ERROR=$((ERROR+1)); fi

	echo "---------- NORMAL MODE ----------"
	CMD="${CMD_NORM}"
	if ! check "1" "1" "ERR" "1" "\$DELETE_IF_OLDER" "1" "1" "1" "4" "${CMD}"; then ERROR=$((ERROR+1)); fi
